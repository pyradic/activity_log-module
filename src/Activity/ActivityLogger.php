...
use Pyro\ActivityLogModule\Activity\Contract\Activity as ActivityContract;
use Pyro\ActivityLogModule\Activity\Exceptions\CouldNotLogActivity;
...
class ActivityLogger
{
    ...
    /** @var \Pyro\ActivityLogModule\Activity\ActivityLogStatus */
    protected $logStatus;

    /** @var \Pyro\ActivityLogModule\Activity\Contract\Activity */
    protected $activity;

    public function __construct(AuthManager $auth, Repository $config, ActivityLogStatus $logStatus)
    {
        $this->auth           = $auth;
        $this->authDriver     = $config->get('pyro.module.activity_log::config.default_auth_driver', $auth->getDefaultDriver());
        $this->defaultLogName = $config->get('pyro.module.activity_log::config.default_log_name');
    ...
    protected function replacePlaceholders(string $description, ActivityContract $activity): string
    {
        return preg_replace_callback('/:[a-z0-9._-]+/i', function ($match) use ($activity) {
            ...
            return $match;
            $attribute = Wrap::stringy($match)->between(':','.')->__toString();
//            $attribute = \Illuminate\Support\Str::between($match, ':', '.');
//            $attribute = (string) (new Str($match))->between(':', '.');
    ...
    protected function getActivity(): ActivityContract
    {
        if ( ! $this->activity instanceof ActivityContract) {
            $this->activity = ActivityLogModule::getActivityModelInstance();
            $this
